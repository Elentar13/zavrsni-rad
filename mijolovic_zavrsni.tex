...
\begin{document}
    \begin{titlingpage}
        ...
        \newpage
            \thispagestyle{empty}
            \includepdf{zavrsni_rad_rjesenje_produljenje.pdf}
        \newpage
            \thispagestyle{empty}
    ...
    \chapter{Uvod}
        ...
        \section{Važnost prognoziranja u inženjerstvu}
            ...
            \begin{figure}[H]
                ...
                \end{tikzpicture}
                \caption{Podaci o učestalosti kvarova brtvi prilikom testnih lansiranja koja su prethodila Challanger-u. Izvor:\cite{NASA}}
                \label{fig:M1}
            \end{figure}
            ...
            \begin{table}[H]
                ...
                \begin{tabular*}{0.9\textwidth}{>{\bfseries}l p{13cm}}
                    ...
                    \textit{\textbf{a,b}} & \textit{konstante dobivene statističkim modeliranjem metodom procjene maksimalne vrijednosti}.\\
                \end{tabular*}
            \end{table}
            % \indent\indent \textit{\textbf{t} \quad temperatura brtve izražena u \textdegree F}\\
            % \indent\indent \textit{\textbf{a, b} \quad konstante dobivene statističkim modeliranjem metodom procjene maksimalne vrijednosti}
    ...
    \chapter{Osnovni pojmovi teorije vjerojatnosti i statistike pri obradi signala}
        ...
        %Signals and Systems DeMYSTiFieD A Self-Teaching Guide (dobro za elektrotehničke primjere autokorelacijske analize)
            ...
            \label{subs:time_series}
                ...
                \footnotetext[1]{Dugoročna tendencija razvoja pojave u vremenu; Niz vrijednosti kroz koje bi promatrana pojava prolazila da nije bilo sezonskih ili slučajnih čimbenika (vremenska razdoblja duža od 5 godina).}
                \footnotetext[2]{Odnosi se na redovne fluktuacije oko trenda koja su sustavna (vremenska razdoblja od jedne i više godine).}
                \footnotetext[3]{Očituje se u sustavnim kretanjima pojave unutar jedne godine.}
                \footnotetext[4]{Izražava nesistematske utjecaje na razvoj pojave \cite{Bahovec}.}
    ...
    \chapter{Autoregresivni modeli}
        ...
        \section{Autoregresijski model (AR)}
            ...
            \footnotetext[5]{Rješenja pripadne karakteristične jednadžbe $\phi(B)$=0.}
            ...
            \subsubsection{Dodatne pretpostavke}
                ...
                \footnotetext[6]{Detaljnije pojašnjenje i izvod dostupni u literaturi \cite{Broersen}.}
            ...
            \subsection{Autoregresijski proces drugog reda -- AR(2) proces}
                ...
                \footnotetext[7]{Funkcija čije trenutne vrijednosti ovise o prethodnim vrijednostima te iste funkcije.}
        ...
        \section{Pomična srednja vrijednost (MA)}
            ...
            % Priestley:\\
            %   Moving Average Processes: Page 135,\\
        ...
        \section{Mješoviti ARMA(p,q) proces}
            ...
            \begin{figure}[H]
                ...
                \begin{tikzpicture}[node distance=2.5cm, auto,>=latex']
                    \centering
                    \node [input, name=input] {};
                    \node [block, align=center, right of=input, node distance=4cm] (ar_component) {$\frac{1}{\phi(B)}$};
                    \node [block, align=center, right of=ar_component, node distance=4cm] (ma_component) {$\theta(B)$};
                    \node [output, right of=sum] (output) {};

                    \path[->] (input) edge node {$\epsilon_t$} (ar_component);
                    \path[->] (ar_component) edge node {$v_t$} (ma_component);
                    \path[->] (ma_component) edge node {$x_t$} (output);
                \end{tikzpicture}
