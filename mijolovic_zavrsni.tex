...
\begin{document}
    ...
    \chapter{Opće metodologije prognoziranja}
        ...
        \section{Prognoze temeljene na statističkim modelima}
            ...
            Naravno, navedeno vrijedi u slučaju jednostavnih prognostičkih modela, te ćemo u nadolazećim poglavljima proučiti detalje kompleksnih nekauzalnih prognostičkih modela temeljenih na vremenskim nizovima.

        \section{Procjena maksimalne vrijednosti}
            ...
            Za razliku od vjerojatnosti, vjerodostojnost možemo opisati kao vrijednost ordinatnog položaja točke $\mathbf{A}$ na ordinati. Kako bismo znali odrediti prethodno navedeni položaj, potrebno je poznavati srednju vrijednost $\mu$ i standardnu devijaciju $\sigma$ promatranog skupa -- temeljem kojih možemo odrediti točku $\mathbf{A}$ kao sjecište pravca okomitog na apcisu u točki $\mu\pm\sigma$ i krivulje normalne distribucije skupa $X$.
            ...
            \begin{figure}[H]
                ...
                \begin{tikzpicture}
                    \begin{axis}[
                        ...
                        \addplot [very thick, domain=-1:0, pattern color=black, pattern=north east lines] {gauss(0,1)} \closedcycle;
                        \addplot [smooth, black, thick] {gauss(0,1)};
                        \node [circle, fill=black, label=60:$\mathbf{A}$, scale=0.3] at (1,0.24) {};
                        \node [rectangle, fill=white, draw, very thick, font=\small, align=center, scale=0.75] at (-0.5,0.15) {\SI{34.135}{\percent}};
                    \end{axis} 
                \end{tikzpicture}
                \caption{Prikaz normalne distribucije}
                \label{fig:31}
            \end{figure}
    ...
    \chapter{Autoregresivni modeli}
        ...
        \section{Autoregresijski model (AR)}
            % Priestley:\\
            %     First Order Autoregressive Process (Linear Markov Process): Page 116\\
            ...
            Autoregresijski $AR(p)$ model je reprezentacija nasumičnog stacionarnog procesa čija izlazna vrijednost u trenutku $x_t$ linearno ovisi o vlastitim vrijednostima u trenucima koji su prethodili, a mogu se definirati kao $x_{t-p}$. Navedeno opće načelo možemo matematički definirati kao

            \begin{equation}
                \epsilon_t=x_t-\mu_x+\phi_1(x_{t-1}-\mu_x)+\phi_2(x_{t-2}-\mu_x)+\cdots+\phi_p(x_{t-p}-\mu_x)+\omega_t,
                \label{eq:51}
            \end{equation}
            gdje su:
            ...
            \begin{table}[H]
                \centering
                \begin{tabular*}{0.9\textwidth}{>{\bfseries}l p{13cm}}
                    \textit{\textbf{$\boldsymbol{\epsilon_t}$}} & \textit{nasumični stacionarni proces}\\
                    \textit{\textbf{$\boldsymbol{x_t}$}} & \textit{član nasumičnog procesa}\\
                    ...
                    \textit{\textbf{$\boldsymbol{\omega_t}$}} & \textit{bijeli šum}\\
                    \textit{\textbf{$\boldsymbol{p}$}} & \textit{red autoregresijskog modela}\\
                \end{tabular*}
            \end{table}

            \textbf{Dodati raspis Leasts Squares approach za $\epsilon_t^2$ ili ono kraće iz Broersena}

            \begin{equation}
                (x_t-\mu_x)=\phi_1(x_{t-1}-\mu_x)+\phi_2(x_{t-2}-\mu_x)+\cdots+\phi_p(x_{t-p}-\mu_x)+\omega_t,
                \label{eq:52}
            \end{equation}

            Jednadžba \ref{eq:52} predstavlja opći eksplicitni prikaz autoregresijskog modela reda $p$. Za tu jednadžbu pretpostaviti ćemo temeljne pretpostavke koje će nam omogućiti jednostaviniju reprezentaciju.
            ...
            \begin{itemize}
                ...
                \begin{equation}
                    x_t=\phi_1x_{t-1}+\phi_2x_{t-2}+\cdots+\phi_px_{t-p}+\omega_t,
                    \label{eq:53}
                \end{equation}
                ...
                \begin{equation}
                    ...
                    \label{eq:54}
                \end{equation}
                gdje $B^px_t=x_{t-p}$ definiramo kao \textbf{backshift operator}. Izvršimo li substituciju \textbf{backshift operatora} u jednadžbama \ref{eq:53} i \ref{eq:54}, dolazimo do sažetijeg izraza za autoregresijski operator
                \begin{equation}
